...
class Walker extends Lint.RuleWalker {
  ...
  constructor(sourceFile: ts.SourceFile, options: Lint.IOptions, private _typeChecker: TypeCheckerWithRelationApi) {
   ...
   */
  private _lintClass(node: ts.ClassDeclaration, sourceClass: ts.ClassDeclaration, expectDeclaredMembers: boolean): void {
    node.members.forEach(member => {
      if (ts.isSetAccessor(member) && usesCoercion(member, this._coercionFunctions) && this._shouldCheckSetter(member)) {
   ...
   * @param expectDeclaredMembers Whether acceptance members should be expected or unexpected.
   */
  private _lintInterfaces(node: ts.ClassDeclaration, sourceClass: ts.ClassDeclaration, expectDeclaredMembers: boolean): void {
      ...
      if (clause.token === ts.SyntaxKind.ImplementsKeyword) {
        clause.types.forEach(clauseType => {
          if (ts.isIdentifier(clauseType.expression)) {
            ...
            if (propNames) {
              propNames.forEach(propName => this.checkStaticMember(sourceClass, propName, expectDeclaredMembers));
            }
          }
        });
      }
  ...
  private _lookupStaticMember(node: ts.ClassDeclaration, propName: string): { memberName: string; memberNode?: ts.PropertyDeclaration } {
