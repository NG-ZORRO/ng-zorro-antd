...
DatePicker、Calendar 和 TimePicker 使用 `NzDateAdapter` 进行日期计算、解析和格式化。日期库 locale 与周规则配置请参考
...
DatePicker、Calendar 和 TimePicker 使用 `NzDateAdapter` 进行日期计算、解析和格式化。`date-fns`、原生 `Date`、locale 配置和自定义日期库请参考
[日期适配器](/docs/date-adapter/zh)。
