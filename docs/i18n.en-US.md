...
`NZ_I18N` for the international copy, and `NZ_DATE_CONFIG` for date-related features. DatePicker, Calendar and
TimePicker use `NzDateAdapter` for date calculation, parsing and formatting. See [Date Adapter](/docs/date-adapter/en)
for date-library locale and week rule configuration.
...
DatePicker, Calendar, and TimePicker use `NzDateAdapter` for date calculation, parsing and formatting. See
[Date Adapter](/docs/date-adapter/en) for `date-fns`, native `Date`, locale configuration and custom date libraries.
