...
@Component({
  ...
  selector: 'hijri-decade-header',
  templateUrl: '../../lib/abstract-panel-header.html',
  encapsulation: ViewEncapsulation.None
})
export class HijriDecadeHeaderComponent extends HijriAbstractPanelHeader {
  override mode: NzDateMode = 'decade';

  get startYear(): number {
    return parseInt(`${this.hijriAdapter.getYear(this.value.nativeDate) / 100}`, 10) * 100;
  }
  ...
  override superPrevious(): void {
    this.changeValue(new CandyDate(this.hijriAdapter.addCalendarYears(this.value.nativeDate, -100)));
  }
