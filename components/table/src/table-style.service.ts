    ...
    combineLatest([this.listOfAutoWidthPx$, this.manualWidthConfigPx$]).pipe(
      map(([autoWidth, manualWidth]) => {
        ...
        if (realManualWidthLength > 0 && realManualWidthLength === autoWidth.length) {
          ...
          // If manualWidth is specified (realManualWidthLength > 0),but manualWidth.length === autoWidth.length
          // and realManualWidthLength !== autoWidth.length (some column width not specified) then return manualWidth to
          // to dynamically change col width.
          return manualWidth;
        }
      })
    )
