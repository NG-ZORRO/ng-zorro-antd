...
@Component({
  selector: 'nz-demo-form-auto-error-tip',
  template: `
    <form nz-form nzAutoErrorTip [nzErrorTipMap]="errorTipMap" [formGroup]="validateForm" (ngSubmit)="submitForm(validateForm.value)">
      <nz-form-item>
      ...
      <nz-form-item>
        ...
        <div>
          <nz-form-control [nzSpan]="12" [nzAutoErrorTip]="false" nzErrorTip="Please input your password!">
      ...
      <nz-form-item>
        ...
        <nz-form-control [nzSpan]="12" [nzAutoErrorTip]="false" [nzErrorTip]="passwordErrorTpl">
...
export class NzDemoFormAutoErrorTipComponent {
  validateForm: FormGroup;

  errorTipMap: Record<string, string> = {
    ...
    new Observable((observer: Observer<MyValidationErrors | null>) => {
      setTimeout(() => {
        if (control.value === 'JasonWood') {
          observer.next({ duplicated: { errorTip: 'The username is redundant!' } });
        } else {
          observer.next(null);
        }
        observer.complete();
...
export class MyValidators extends Validators {
  ...
  static maxLength(maxLength: number): ValidatorFn {
    return (control: AbstractControl): MyValidationErrors | null => {
      ...
      return { maxlength: { errorTip: `MaxLength is ${maxLength}` } };
    };
  }
