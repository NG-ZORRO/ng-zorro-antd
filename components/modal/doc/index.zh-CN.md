...
| nzComponentParams   | 即将废弃，将在下一个主要版本删除，请使用`nzData`。当 `nzContent` 为组件时将作为实例属性，为 `TemplateRef` 时将作为模版变量 | `object`                                                                                                                                                  | `object` | - |
| nzData              | 当 `nzContent` 为组件时请将`NZ_MODAL_DATA`注入到组件内，为 `TemplateRef` 时将作为模版变量                                                                                                             | `object` | - |
| nzIconType          | 图标 Icon 类型。<i>仅 确认框模式 下有效</i>                                                                                                                                                  | `string` | `'question-circle'` |
| nzAutofocus         | 自动聚焦及聚焦位置，为 `null` 时禁用                                                                                                                                                         | `'ok' \| 'cancel' \| 'auto' \| null` | `'auto'` |

#### NZ_MODAL_DATA

> `NZ_MODAL_DATA` 是一个[InjectionToken](https://angular.io/api/core/InjectionToken)。当你需要传递数据给自定义组件时，可将此令牌注入到你的自定义组件即可。

```ts
import { NzModalService, NzModalModule, NZ_MODAL_DATA } from 'ng-zorro-antd/modal'
...
export class AppComponent {
  ...
  open(){
    this._nzModalService.create<MyModalComponent, ModalData, ModalResult>({
      nzContent: MyModalComponent,
      nzData: {
        name: 'Jack'
      },
    }).afterClose.subscribe(console.log) // Result: {name: "Jack ❤️ angular"}
  }
}
