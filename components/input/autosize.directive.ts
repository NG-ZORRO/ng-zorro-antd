...
export class NzAutosizeDirective implements AfterViewInit, DoCheck {
  ...
  resizeToFitContent(force: boolean = false): void {
    ...
    // Reset the textarea height to auto to shrink back to its default size.
    ...
    if (typeof requestAnimationFrame !== 'undefined') {
      this.ngZone.runOutsideAngular(() =>
        requestAnimationFrame(() => {
          ...
          /*TODO
            In the next version of Angular (20.1), it will be possible to check with destroyRef is a component is destroyed
            thanks to the isDestroyed property. At this moment we have to change the condition
           */
          if (this.el && document.activeElement === textarea) {
